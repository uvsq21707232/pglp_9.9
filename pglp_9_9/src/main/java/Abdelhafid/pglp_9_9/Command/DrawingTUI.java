...
/**
 * Classe qui gére l'intéraction avec l'utilisateur.
 */
public class DrawingTUI {
	...
	private Forme CommandCercleCreate(final String[] split) {
		...
		if (!chaine[0].equals("") || !(chaine[1].startsWith("(") && chaine[1].endsWith(")"))) {

			System.out.println("Votre commande est invalide ou " + "vous manque une parenthese);");
			...
			if (chaine.length == 3) {
				...
				System.out.println("commande invalide. " + " Veuillez verifiez le nombre d'argument de votre commande");
			}
    ...
    */
	...
	/**
	 * Fonction qui analyse la commande de création d'un groupe.
	 * @param split La commande siasit par l'utilisateur en format tableau.
	 * @return un Groupe si la commande est valide.
	 */
	private Forme CommandGroupeCreate(final String[] split) {

		String[] chaine = split[1].split("Groupe");
		if (!chaine[0].equals("") || !(chaine[1].startsWith("(") && chaine[1].endsWith(")"))) {
			...
			Composite grp = new Composite(split[0]);

			for (String s : chaine) {
				Forme forme = this.recherche(s);
				if (forme != null) {
					grp.add(forme);
				} else {
					System.err.println("Commande invalide ou forme à ajouter non trouvé");
	...
	public Command nextCommand(String command) {
		if (command.contains("=")) {
			...
			return this.CommandRemove(command);
		} else if (!command.equalsIgnoreCase("exit")) {
			System.err.println("la commdande n'est pas reconnu \n");
		}
		return null;
	}
	/**
    *  Foction quicrée une forme.
    * @param cmd Le commande saisit par l'utilisateur.
    * @return La forme si trouvée.
    */
	private Forme Creation(String cmd) {
		String[] split;
		split = cmd.split("=");
		...
		if (split[0].contains(" ")) {
			...
			if (split[1].contains("Carre")) {
				forme = CommandCarreCreate(split);
			} else if (split[1].contains("Cercle")) {
				forme = CommandCercleCreate(split);
			} else if (split[1].contains("Rectangle")) {
				forme = CommandRectangleCreate(split);
			} else if (split[1].contains("Triangle")) {
				forme = CommandTriangleCreate(split);
			} else if (split[1].contains("Groupe")) {
				forme = CommandGroupeCreate(split);
			}
     ...
     */
	private Command CommandMove(final String cmd) {

		String chaine = cmd.replace(" ", "");
		String[] chaine1 = chaine.split("move");

		if (!chaine1[0].equals("") || !(chaine1[1].startsWith("(") && chaine1[1].endsWith(")"))) {
			...
			if (chaine1.length == 3) {
				...
				System.out
						.println("commande invalide. " + " Veuillez verifiez le nombre d'arguments de votre commande");
			}
    ...
    */
	private Command CommandRemove(String cmd) {
		...
		String chaine = cmd.replace(" ", "");
		String[] chaine1 = chaine.split("delete");
	...
	/**
	 * Fonction qui affiche l'ensemble des Creations.
     */
	public void dessiner() {
		...
		System.out.println("*********** Vos Créations ********************");
